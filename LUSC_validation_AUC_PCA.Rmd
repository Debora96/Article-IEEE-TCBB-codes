...
library(finalfit)
# Crosstable 
explanatory = c("obs.time", "age", "gender", "neoplasm", "metastasis", "ajcc.stage")
dependent = 'status'
data %>%
...
for(sign in gsign.lusc$id[c(1:6)]) {
  print(sign)
  col_sign <- intersect(gsign_lusc[sign, "signature"][[1]], names(data))
...
for(sign in gsign_lusc$id[c(1:8)]) {
  #print(sign)
  col_sign <- intersect(gsign_lusc[sign, "signature"][[1]], names(data))
